...
class Article(TimeStampedUUIDModel):
    ...
    def __str__(self):
        return f"{self.author.username}'s article {self.slug}"
    ...
    def article_read_time(self):
        read_engine = ArticleReadTimeEngine(self)
        return read_engine.get_read_time()
...
class ArticleViews(TimeStampedUUIDModel):
    ...
    article = models.ForeignKey(to=Article, related_name="article_views", on_delete=models.CASCADE)
